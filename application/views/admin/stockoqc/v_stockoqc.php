...
<div class="container-fluid">
	...
	<div class="card">
		<div class="card-body">
			...
			<?= form_close(); ?>
			<a href="" class="btn btn-success btn-sm mb-3" data-toggle="modal" data-target="#add"><i class="fa fa-plus"></i> Tambah Stock OQC</a>
					...
					<tbody>
						...
						foreach ($list_stockoqc as $item) { ?>
							<tr>
								...
								<td class="text-center"><?= $item->stockoqc_semi; ?></td>
								<td class="text-center"><?= $item->stockoqc_finish; ?></td>
								<td class="text-center"><?= $item->stockoqc_remain; ?></td>
								<td class="text-center">
									<div class="form-group card ">
										<a href="<?= base_url('view_stockoqc_edit/' . $item->stockoqc_id); ?>" class="btn btn-light btn-sm"><i class="fa fa-edit"></i> Edit</a>
									</div>
									<div class="form-group card ">
										<a href="<?= base_url('process_stockoqc_delete/' . $item->stockoqc_id); ?>" class="btn btn-danger btn-sm" onclick="return confirm('Yakin menghapus data?')"><i class="fa fa-trash"></i> Hapus</a>
									</div>
								</td>
							</tr>
						<?php } ?>
		...
		<div class="modal-content">
			...
			<div class="modal-body">
				...
				<div class="form-group">
					...
					<input type="text" name="stockoqc_kode" class="form-control" id="stockoqc_kode" value="<?= "PA-" . date('Ymd') . $kodeTambah . $kode ?>" disabled>
					<input type="hidden" name="stockoqc_kode" class="form-control" id="stockoqc_kode" value="<?= "PA-" . date('Ymd') . $kodeTambah . $kode ?>">
				...
				<div class="form-group">
					<label for="stockoqc_hasil"> Hasil Stock OQC</label>
					<input type="number" name="stockoqc_hasil" class="form-control form-control-user" id="stockoqc_hasil" placeholder="Masukan hasil Stock OQC" value="<?= set_value('stockoqc_hasil'); ?>" required>
					Jika tidak perlu masukkan nilai nol "0".
					<?= form_error('stockoqc_hasil', '<small class="text-danger" ><b>', '</b></small>') ?>
				</div>
