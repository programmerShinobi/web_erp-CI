...
class M_podelivery extends CI_Model {
  protected $response = [];
	...
	public function get_view($start,$end)
	{
		if ($start == null || $end == null) {
			...
			return $this->db->select("*")
				->from("tb_podelivery")
				->join("tb_purchaseorder", "tb_podelivery.purchaseorder_id = tb_purchaseorder.purchaseorder_id")
				...
				->order_by("tb_podelivery.podelivery_id", "DESC")
				->get();
		} else {
			return $this->db->select("*")
				->from("tb_podelivery")
