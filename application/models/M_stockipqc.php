...
class M_stockipqc extends CI_Model {
  protected $response = [];
	...
	public function get_view($start, $end)
	{
		if ($start == null || $end == null) {
			...
			return $this->db->select("*")
				->from("tb_stockipqc")
				->join("tb_purchaseorder", "tb_stockipqc.purchaseorder_id = tb_purchaseorder.purchaseorder_id")
				...
				->order_by("tb_stockipqc.stockipqc_id", "DESC")
				->get();
		} else {
			return $this->db->select("*")
				->from("tb_stockipqc")
