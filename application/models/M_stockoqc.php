...
class M_stockoqc extends CI_Model {
  protected $response = [];
	...
	public function get_view($start, $end)
	{
		if ($start == null || $end == null) {
			...
			return $this->db->select("*")
				->from("tb_stockoqc")
				->join("tb_purchaseorder", "tb_stockoqc.purchaseorder_id = tb_purchaseorder.purchaseorder_id")
				...
				->order_by("tb_stockoqc.stockoqc_id", "DESC")
				->get();
		} else {
			return $this->db->select("*")
				->from("tb_stockoqc")
